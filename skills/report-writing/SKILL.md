...
## MITRE ATLAS MAPPING (AI / agentic findings)

For findings against AI/ML or agentic systems, add a **MITRE ATLAS** technique line
(atlas.mitre.org) next to the CWE — it hands triagers the standard adversarial-AI
taxonomy. Put `atlas_ids` in `GATE.json` (the preflight echoes it) and a
`## MITRE ATLAS Mapping` line in the draft. `report_generator.py` auto-fills it from
the vuln_type (`ATLAS_MAP`).

| Finding class | ATLAS technique(s) |
|---|---|
| Direct prompt injection / jailbreak | `AML.T0051` / `AML.T0051.001` |
| Indirect prompt injection (untrusted content → agent) | `AML.T0051.002` |
| Agent tool abuse / excessive agency | `AML.T0061` AI Agent Tools, `AML.T0062` Exfil via Tool Invocation |
| Agent context / RAG poisoning | `AML.T0058`, `AML.T0020` |
| Training-data / inference exfil | `AML.T0024` |

Classic web/app bugs (SSRF, SQLi, XSS, IDOR) have **no ATLAS cell** — omit the mapping
unless the sink is an AI tool, in which case map the agentic impact (e.g. SSRF as the
transport for `AML.T0051.002` / agent data-exfil).
...
## PENTEST / RED-TEAM ENGAGEMENT REPORT TEMPLATE

Use this for a **multi-finding engagement** (pentest / red-team / authorized assessment) — NOT a
single bug-bounty submission. The SeethaAi dashboard parses this exact shape (`tools/report_struct.py`)
to render a per-finding severity table, an attack-surface block, the engagement metadata, and ATT&CK
chips. Write `reports/<engagement-id>/REPORT.md` to this structure; siblings `CLOSEOUT.md` / `SCOPE.md`
are surfaced as a doc-switcher.

```markdown
# <Target> — <Red Team | Penetration Test> Engagement Report (<Phase/scope>)

**Engagement:** <authorization / NDA note>. **Date:** YYYY-MM-DD.
**Scope:** <hosts / ranges (see SCOPE.md)>. **Tester:** <name>. **Status:** <phase / blockers>.

---

## Executive summary
<2–4 sentences: overall posture, headline risk, what's confirmed vs pending.>

## Attack surface map (recon)
| Host | Hosting | Role | Exposure |
|---|---|---|---|
| … | … | … | … |

## Findings
### F1 — <concise title>  · **<Critical|High|Medium|Low|Informational>**
**ATT&CK:** T1190, T1078            <!-- optional; one or more technique IDs -->
**Impact:** <business impact, quantified where possible>
**Remediation:** <specific fix>
**Evidence:** <path under evidence/ or recon/>

### F2 — <title>  · **<Severity>**
…

## Verified and ruled out (no finding — verify-before-rating)
- <thing tested that was NOT vulnerable, with why>

## Defensive strengths (verified)
- <controls that held>

## Next steps (Phase 2 / what we need)
<authenticated tests, credentials required, follow-on scope.>
```

**Severity notation — three accepted forms (all parsed):**
- heading trailer: `### F1 — title  · **Low**`  (preferred)
- parenthetical:  `### F1 — title (LOW, confirmed)`
- severity-grouped sections + ID prefix: `## High Severity Findings` → `### H-01: title`

**Conditional findings:** if a finding's severity depends on a later phase, say so in the severity
(`· **Needs Phase 2 to confirm (potential High/Critical)**`) or body (`potential` / `needs … confirm`).
The dashboard tags these "conditional" and **excludes them from the headline severity** — so an
unconfirmed maybe-Critical never inflates the engagement's posture. Finding IDs `F<n>` (or
`C-/H-/M-/L-##`) are required; one heading per finding.
